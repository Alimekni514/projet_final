...
export default function MediaContainer({ img, title, rate, desc,id }) {
  ...
  const  HandleWatchList= async()=>{
    ...
    const list = await fetch(`https://api.themoviedb.org/3/list/${localStorage.getItem('Watchlistid')}/add_item?api_key=b3ec5aad46e51258856256128c47b00c&session_id=7681eb6a4de93e5f987a99934963029a4166328d`,
    {
      method:'POST',
  ...
  const HandleRate=async()=>{
    const list = await fetch(`https://api.themoviedb.org/3/list/${localStorage.getItem('ratelistid')}/add_item?api_key=b3ec5aad46e51258856256128c47b00c&session_id=7681eb6a4de93e5f987a99934963029a4166328d`,
    {
      method:'POST',
    headers:{'Content-Type': 'application/json'},
    body: JSON.stringify({
    ...
    return (
        <div className='movie'>
            ...
            <div className='media-info'>
                <h3> {title}</h3>
                <span> {rate} </span>
                { <input style={{display:'none'}} type="text" defaultValue={id} ref={mediaid} /> }
            </div>
